...
function getBlockTextClass(type) {
  switch (type) {
    ...
    case "communication_label":
      return "text-center font-semibold uppercase underline underline-offset-4";
...
// function applyNumberFormatting(value) {
...
//       if (!trimmed) return "";
//       return `    ${index + 1}. ${trimmed.replace(/^\s*\d+\.\s*/, "")}`;
//     })
//     .join("\n");
// }
...
function renderSubjectBlock(block, onBlockFocus, onBlockChange, styling) {
  return (
    <div className="rounded-md px-1">
      <div className="flex items-start gap-2">
        ...
        <textarea
          ...
          placeholder={block.placeholder || "Enter subject..."}
          rows={getRowsForBlock(block.type, block.content)}
          className="w-full resize-none overflow-hidden border-none bg-transparent p-0 text-justify font-bold text-slate-900 outline-none placeholder:text-slate-400"
          style={{
            lineHeight: getBlockLineHeight(block.type, styling),
            minHeight: "26px",
          }}
          ref={(el) => adjustTextareaHeight(el, false)}
        />
      </div>
    </div>
  );
            ...
            : block.type === "government_identity" ||
              ...
              : block.type === "document_number" || block.type === "do_number"
                ...
                : block.type === "signature_block"
                  ? "12px"
                  : "18px",
        textIndent: isBody ? `${draftStyling.bodyFirstLineIndent || 0.3}in` : 0,
      }}
    />
  );
}
            ...
            >
              ...
              <div
                className={`rounded-md px-1 py-0 transition ${
                  active ? "bg-slate-50/70" : "bg-transparent"
                }`}
              >
                {block.type === "place_date_line" ? (
                  ...
                  renderSubjectBlock(block, onBlockFocus, onBlockChange)
                ) : (
                  <>
