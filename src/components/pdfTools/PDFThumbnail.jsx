...
export default function PDFThumbnail({ file, buffer }) {
  ...
  useEffect(() => {
    if (!file || !buffer) return;
    if (typeof window === "undefined") return; // crawler/SSR safety
    ...
    (async () => {
      try {
        ...
        // ✅ Lazy imports (prevents Google render crash at module load time)
        const pdfjsLib = await import("pdfjs-dist/build/pdf.mjs");
        const workerMod =
          await import("pdfjs-dist/build/pdf.worker.min.mjs?raw");
        const pdfWorkerSrc = workerMod.default;

        const blob = new Blob([pdfWorkerSrc], {
          type: "application/javascript",
        });
        workerUrl = URL.createObjectURL(blob);

        // pdfjsLib is ESM module namespace; default export not used here
        pdfjsLib.GlobalWorkerOptions.workerSrc = workerUrl;
        ...
        const cssWidth = window.innerWidth < 640 ? 112 : 128;
        const dpr = window.devicePixelRatio || 1;
