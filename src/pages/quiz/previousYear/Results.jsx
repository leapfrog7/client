...
const FULL_DURATION = 2 * 60 * 60; // 2 hours (seconds)
...
// robust answered check (treat "", null, undefined as unanswered)
const isAnswered = (r) => {
  if (!r) return false;
...
export default function Results({
  ...
  // freeze remaining time at first render of results
  ...
  // synthesize one response per question; if missing => unanswered stub
  const fullResponses = useMemo(() => {
    return safeQuestions.map((q, idx) => {
      ...
      return {
        questionId: qid,
        userAnswer: null,
        isCorrect: false, // meaningless for unanswered
  ...
  // metrics must derive from the same source used for rendering
  const { correct, wrong, unanswered, attempted, accuracy } = useMemo(() => {
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mt-8 flex flex-wrap gap-3 justify-center">
        ...
        {typeof onRetake === "function" && (
          <button
            className="px-5 py-2 hidden bg-pink-600 text-white rounded-lg hover:bg-pink-700"
            onClick={onRetake}
          >
            Retake This Paper
          </button>
